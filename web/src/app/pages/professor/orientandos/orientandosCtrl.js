(function () {
	...
	function OrientandosCtrl($scope, $apiService, $modalservice, $PROFESSOR, $uibModal, $timeout) {
		...
		function VerEtapasCtrl($scope, param) {
			...
			$scope.VerFeedbacks = VerFeedbacks;
		}
		...
		function VerFeedbacks(projeto) {
			$apiService.projeto.GetFeedbacks(projeto.aluno._id)
			.then(function (response){
				var feedbacks = response.data; 
				$modalservice.executar({
					data: { feedbacks: feedbacks},
					size: 'lg',
					template: 'app/pages/componentes/feedback/feedbacks.html'
				});
